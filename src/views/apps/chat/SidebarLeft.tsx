...
const SidebarLeft = (props: ChatSidebarLeftType) => {
  ...
  const [chatsCache, setChatCache] = useState<ChatType[]>([])
  ...
  // ** Hooks
  const renderChats = () => {
    return chatsCache.map((chat: ChatType, index: number) => {
  ...
  const handleFilter = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    setChatCache(
      chats.filter(
        chat =>
          chat.user.firstName.toLocaleLowerCase().includes(value) ||
          chat.user.lastName.toLocaleLowerCase().includes(value)
      )
    )
  }
